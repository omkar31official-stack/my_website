...
export default function ProcessSection() {
  const steps = [
    ...
    },
    {
      ...
      icon: <ClipboardList size={24} className="text-rose-400" />,
    },
    {
      title: "Design",
  ...
  return (
    <section id="process" className="py-28 md:py-36 bg-slate-950 relative border-t border-white/5 overflow-hidden">
      <div className="absolute left-1/2 top-0 w-[600px] h-[600px] -translate-x-1/2 -translate-y-1/2 bg-red-900/10 blur-[150px] pointer-events-none rounded-full" />
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="absolute left-[28px] md:left-1/2 top-4 bottom-4 w-[2px] bg-white/10 -translate-x-1/2 hidden md:block">
            <motion.div
              ...
              className="w-full bg-gradient-to-b from-red-500 via-rose-500 to-orange-500"
            />
          </div>
          ...
          <div className="space-y-12 md:space-y-0 relative">
            {steps.map((step, idx) => (
              <div key={idx} className={`relative flex items-center md:items-start flex-col md:flex-row gap-6 md:gap-0 mt-8 first:mt-0 ${
                ...
                >
                  <div className="bg-slate-900 border border-white/10 p-8 sm:p-10 rounded-[2rem] shadow-xl w-full hover:border-red-500/30 transition-colors group relative overflow-hidden">
                ...
                <motion.div
                  ...
                  className="absolute md:static left-0 top-6 w-14 h-14 rounded-full bg-slate-950 border-4 border-slate-900 shadow-[0_0_15px_rgba(239,68,68,0.5)] z-10 flex items-center justify-center shrink-0 mx-auto md:mt-2 self-start"
                >
                  {step.icon}
                </motion.div>
